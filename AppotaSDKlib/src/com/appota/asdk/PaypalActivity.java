...
import com.appota.asdk.model.ResultDelegate;
...
public class PaypalActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setTitle(R.string.paypal_activity_title);
		Thread libraryInitializationThread = new Thread() {
			public void run() {
				initLibrary();
				Bundle bundle = getIntent().getExtras();
				paypal = (PaypalPayment) bundle.getSerializable(Constant.PAYPAL_DATA);
		...
		drawUI();
		progressDialog = ProgressDialog.show(this, getResources().getString(R.string.loading), "Please wait...", true);
	}
	...
	private void drawUI() {
		...
		layoutPayment.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
	...
	public void showFailure() {
		Toast.makeText(getApplicationContext(),"Could not initialize the PayPal library.", Toast.LENGTH_SHORT).show();
	}
	...
	private PayPalPayment getPayment() {
		...
		payment.setCurrencyType(paypal.getForm().getCurrencyCode());
		// Sets the recipient for the payment. This can also be a phone number.
		payment.setRecipient(paypal.getForm().getBusiness());
		// Sets the amount of the payment, not including tax and shipping
		// amounts.
		payment.setSubtotal(new BigDecimal(paypal.getForm().getAmount()));
		...
		item1.setName("Nap tien vao AppStore.vn");
		...
		item1.setTotalPrice(new BigDecimal(paypal.getForm().getAmount()));
		// Sets the unit price.
		item1.setUnitPrice(new BigDecimal(paypal.getForm().getAmount()));
		// Sets the quantity.
		item1.setQuantity(1);
		...
		// Company.
		payment.setMerchantName("Nap tien vao AppStore.vn");
		...
		payment.setMemo("Nap tien vao AppStore.vn");
		return payment;
	}
	...
	public void onBackPressed() {
		// TODO Auto-generated method stub
		setResult(RESULT_OK, getIntent().putExtra(Constant.TRANSACTION_ID_KEY, paypal.getTransactionId()));
		super.onBackPressed();
	}
}
